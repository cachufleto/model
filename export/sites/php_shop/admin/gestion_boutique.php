	...
	if($ref_article->num_rows >0 && isset($_GET['action']) && $_GET['action'] == 'ajout') // si la référence existe
	{
		...
		if(!empty($_FILES['photo']['name'])) // si une photo a été posté
		{
			...
			if(verificationExtensionPhoto())
			{
				...
				$photo_bdd = RACINE_SITE . "photo/$nom_photo"; // chemin src que l'on enregistre dans la BDD
				$photo_dossier = RACINE_SERVER . RACINE_SITE . 'photo/'. $nom_photo; // chemin pour l'enregistrement du fichier dans le dossier cible via la fonction copy()
