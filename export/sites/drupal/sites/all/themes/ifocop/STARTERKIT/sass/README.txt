...
This directory includes Sass versions of ifocop's CSS files.
...
ifocop 7.x-5.0 was developed with the latest version of Sass and Compass (at the
time!) Newer versions are not compatible with ifocop's Sass files. To ensure you
are using the correct version of Sass and Compass, you will need to use the
"bundle" command which will read ifocop's Gemfile to ensure the proper versions are
...
ifocop can be set to generate sourcemap files. Edit config.rb, and uncomment:
