...
/**
 * @file
 * Contains functions to alter Drupal's markup for the ifocop theme.
 *
 * IMPORTANT WARNING: DO NOT MODIFY THIS FILE.
 *
 * The base ifocop theme is designed to be easily extended by its sub-themes. You
 * shouldn't modify this or any of the CSS or PHP files in the root ifocop/ folder.
 * See the online documentation for more information:
 *   https://drupal.org/documentation/theme/ifocop
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('ifocop_rebuild_registry') && !defined('MAINTENANCE_MODE')) {
  // Rebuild .info data.
 ...
 */
function ifocop_theme(&$existing, $type, $theme, $path) {
  include_once './' . drupal_get_path('theme', 'ifocop') . '/ifocop-internals/template.theme-registry.inc';
  return _ifocop_theme($existing, $type, $theme, $path);
}
 ...
 */
function ifocop_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  $output = '';

  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('ifocop_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('ifocop_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = filter_xss_admin(theme_get_setting('ifocop_breadcrumb_separator'));
      $trailing_separator = $title = '';
      if (theme_get_setting('ifocop_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('ifocop_breadcrumb_trailing')) {
 ...
 *   also be "maintenance_page" since ifocop_preprocess_maintenance_page() calls
 *   this function to have consistent variables.
 */
function ifocop_preprocess_html(&$variables, $hook) {
  // Add variables and paths needed for HTML5 and responsive support.
  $variables['base_path'] = base_path();
  $variables['path_to_ifocop'] = drupal_get_path('theme', 'ifocop');
  // Get settings for HTML5 and responsive support. array_filter() removes
  // items from the array that have been disabled.
  $html5_respond_meta = array_filter((array) theme_get_setting('ifocop_html5_respond_meta'));
  ...
  // If the user is silly and enables ifocop as the theme, add some styles.
  if ($GLOBALS['theme'] == 'ifocop') {
    include_once './' . $variables['path_to_ifocop'] . '/ifocop-internals/template.ifocop.inc';
    _ifocop_preprocess_html($variables, $hook);
  }
  ...
  $variables['skip_link_anchor'] = check_plain(theme_get_setting('ifocop_skip_link_anchor'));
  $variables['skip_link_text']   = check_plain(theme_get_setting('ifocop_skip_link_text'));
  ...
  if (theme_get_setting('ifocop_wireframes')) {
 ...
 */
function ifocop_process_html(&$variables, $hook) {
 ...
 */
function ifocop_process_html_tag(&$variables) {
 ...
 */
function ifocop_html_head_alter(&$head) {
 ...
 */
function ifocop_preprocess_page(&$variables, $hook) {
 ...
 */
function ifocop_preprocess_maintenance_page(&$variables, $hook) {
  ifocop_preprocess_html($variables, $hook);
  // There's nothing maintenance-related in ifocop_preprocess_page(). Yet.
  //ifocop_preprocess_page($variables, $hook);
}
 ...
 *   The name of the template being rendered ("maintenance_page" in this case.)
 */
function ifocop_process_maintenance_page(&$variables, $hook) {
  ifocop_process_html($variables, $hook);
 ...
 */
function ifocop_preprocess_node(&$variables, $hook) {
 ...
 */
function ifocop_preprocess_comment(&$variables, $hook) {
 ...
 */
function ifocop_preprocess_region(&$variables, $hook) {
  // Sidebar regions get some extra classes and a common template suggestion.
  if (strpos($variables['region'], 'sidebar_') === 0) {
    $variables['classes_array'][] = 'column';
    $variables['classes_array'][] = 'sidebar';
    // Allow a region-specific template to override ifocop's region--sidebar.
  ...
  elseif ($variables['region'] == 'content') {
    // Allow a region-specific template to override ifocop's region--no-wrapper.
 ...
 */
function ifocop_preprocess_block(&$variables, $hook) {
  ...
  // The last_in_region property is set in ifocop_page_alter().
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function ifocop_process_block(&$variables, $hook) {
 ...
 */
function ifocop_page_alter(&$page) {
 ...
 */
function ifocop_form_node_form_alter(&$form, &$form_state, $form_id) {
 ...
 * @ingroup themeable
 */
function ifocop_menu_local_tasks(&$variables) {
  $output = '';
 ...
 * @ingroup themeable
 */
function ifocop_menu_local_task($variables) {
  $type = $class = FALSE;

  $link = $variables['element']['#link'];
  $link_text = $link['title'];

  // Check for tab type set in ifocop_menu_local_tasks().
 ...
 */
function ifocop_preprocess_menu_link(&$variables, $hook) {
 ...
 */
function ifocop_status_messages($variables) {
 ...
 */
function ifocop_mark($variables) {
  $type = $variables['type'];
 ...
 */
function ifocop_panels_default_style_render_region($variables) {
